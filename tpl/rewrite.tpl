...
<!-- breadcrumb -->
				<div class="row breadcrumb">
					<ol class="breadcrumb-entry">
						...
						<li class="active"><a href="?mod=rewrite">{{ lang['rewrite'] }}</a></li>
					</ol>
				</div>
				<!-- /breadcrumb -->
<div class="row">
...
<tr id="row.editRow" valign="top" class="rewriteEditLine">
 ...
 <td id="row.url"><input type="text" class="form-control" id="ed.regex" /><br/>
 	{{ lang['tbl.available_vars'] }}:<br/><span id="ed.varlist"></span>
 </td>
 <td id="row.flags">
 <input id="ed.flagPrimary" type="checkbox"/> <input id="ed.flagFailContinue" type="checkbox"/> 
 <input id="ed.flagDisabled" type="checkbox"/>
 </td>
 <td nowrap>
 <button type="button" class="btn btn-warning" onclick="reSubmitEdit();" id="ed.button">Add</button>
 <button type="button" class="btn btn-warning" id="ed.bcancel" onclick="reCancelEdit();">Cancel</button>
</td>
</tr>
					...
					<div class="col-md-12">
						<div class="panel panel-default">
							<div class="panel-body">
								<button type="button" class="btn btn-primary" onclick="reServerSubmit();">Сохранить</button> 
							</div>
						</div>
					</div>
				</div>
...
function reFillCmd(plugin) {
 ...
 tmp = '<select name="ed.cmd" class="form-control" id="ed.cmd" onchange="reUpdateDescr(document.getElementById(\'ed.pluginName\').value, this.value);">';
...
// Init editing params
{
 ...
 tmp = '<select name="ed.pluginName" class="form-control" id="ed.pluginName" onchange="reFillCmd(this.value);">';
