...
<div class="panel-body">
						...
						[status]<button type="button" class="btn btn-info" onmousedown="javascript:ChangeOption('categories')">{l_categories}</button>[/status]
...
<span class="input-group-addon">{l_show_preview} <input type="checkbox" onclick="setCookie('img_preview',this.checked?1:0); document.location=document.location;" {box_preview}/></span>
<span class="input-group-addon">{l_per_page}</span> <input class="form-control" name="npp" value="{npp}" type="text" size="3" /><span class="input-group-btn"><button type="submit" class="btn btn-success">{l_show}</button>
										</span>
</div>
										</div>
<div style="margin: 10px 0;">
...
<span class="input-group-addon">{l_month}</span> 
	<select class="form-control" name="postdate"><option selected value="">- {l_all} -</option>{dateslist}</select>


    [status]<span class="input-group-addon">{l_author}</span>  
			<select class="form-control" name="author"><option value="">- {l_all} -</option>{authorlist}</select>[/status]
										...
										<div style="margin: 10px 0;">
									<div class="input-group">
										<span class="input-group-addon">{l_category}</span> {dirlistcat}
										</div>
										</div>
...
{entries}
<tr>
<td colspan="12" class="contentEdit">{pagesss}</td>
</tr>
</table>
<div class="row">
<div class="col-md-8">
					<div class="panel-body">
									<div class="input-group">
								[status]<span class="input-group-addon">
											{l_move}:
										</span>
										{dirlist}
								...
								<div class="col-md-4">
								<div class="panel-body">
							[status]<button class="btn btn-success" type="submit" onclick="setStatus('delete');">{l_delete}</button>[/status]
								</div>
								</div>
							</div>
							</div>
...
<div style="margin: 10px ;">
									<div class="input-group">
<input type="text" class="form-control" name="newfolder" size="30" /><span class="input-group-btn">
...
<div style="margin: 10px ;">
<div class="input-group">
{dirlist}<span class="input-group-btn"> <button type="submit" class="btn btn-success">OK</button>
										</span>
									</div>
								</div>
								</div>
