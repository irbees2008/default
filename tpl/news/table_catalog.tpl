    ...
    <tr>
	    <td valign="top">
            <table class="table">
                ...
                <tbody>
                    <tr>
					    <td {% if (cat_active < 1) %}style="background-color: #EEEEEE;"{% endif %}><img alt="+" height="18" width="18" src="{{ skins_url }}/images/catmenu/plus.gif" /> <a href="?mod=news">Все категории</a></td>
					</tr>
{% for cat in catmenu %}
                    <tr>
                       <td {% if (cat.flags.selected) %}style="background-color: #EEEEEE;"{% endif %}>
					      <div style="float: left; margin-right: 5px;">{{ cat.cutter }}</div> 
						  <div style="float: left;"><a href="?mod=news&category={{ cat.id }}">{{ cat.name }}</a>{% if (cat.posts>0) %} [ {{ cat.posts }}]{% endif %}</div>
						</td>
                    </tr>
{% endfor %}
                </tbody>
