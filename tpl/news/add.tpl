...
{% endif %}
  {% if (not isBBCode) %}<div style="margin: 10px 0;">{{ quicktags }}</div><div style="margin: 10px 0;"> {{ smilies }}</div>{% else %}{% endif %}
  <div style="margin: 10px 0;">
{% if (flags.edit_split) %}
	<div id="container.content.short" class="contentActive"><textarea style="width: 99%; padding: 1px; margin: 1px;" onclick="changeActive('short');" onfocus="changeActive('short');" name="ng_news_content_short" {% if (isBBCode) %}class="{{ attributBB }}"{% else %}id="ng_news_content_short"{% endif %} rows="10" tabindex="2"></textarea></div>
{% if (flags.extended_more) %}    <table cellspacing="2" cellpadding="0" width="100%"><tr><td nowrap>{{ lang.addnews['editor.divider'] }}: &nbsp;</td><td style="width: 90%"><input tabindex="2" type="text" name="content_delimiter" style="width: 99%;" value=""/></td></tr></table>{% endif %}
	<div id="container.content.full" class="contentInactive"><textarea style="width: 99%; padding: 1px; margin: 1px;" onclick="changeActive('full');" onfocus="changeActive('full');" name="ng_news_content_full" {% if (isBBCode) %}class="{{ attributBB }}"{% else %}id="ng_news_content_full"{% endif %} rows="10" tabindex="2"></textarea></div>
{% else %}
...
{% if (flags.meta) %}
    <div style="margin: 10px 0;">
   <textarea name="description" class="form-control" placeholder="{{ lang.addnews['description'] }}"  cols="80"></textarea>
   </div>
   <div style="margin: 10px 0;">
   <textarea id="newsKeywords" class="form-control" placeholder="{{ lang.addnews['keywords'] }}" name="keywords" cols="80"></textarea>
  </div>
{% endif %}
...
{% endif %}
  <div class="panel panel-default">
							...
							<div class="panel-footer">
  <div class="list">
  <input type="checkbox" name="mainpage" value="1" class="check" id="mainpage" {% if (flags.mainpage) %}checked="checked" {% endif %}{% if flags['mainpage.disabled'] %}disabled {% endif %}  /><label for="mainpage"></label> {{ lang.addnews['mainpage'] }}<br />
  <input type="checkbox" name="pinned" value="1" class="check" id="pinned" {% if (flags.pinned) %}checked="checked" {% endif %}{% if flags['pinned.disabled'] %}disabled {% endif %}  /><label for="pinned"></label>{{ lang.addnews['add_pinned'] }}<br />
  <input type="checkbox" name="catpinned" value="1" class="check" id="catpinned" {% if (flags.catpinned) %}checked="checked" {% endif %}{% if flags['catpinned.disabled'] %}disabled {% endif %}  /><label for="catpinned"></label>{{ lang.addnews['add_catpinned'] }}<br />
  <input type="checkbox" name="favorite" value="1" class="check" id="favorite" {% if (flags.favorite) %}checked="checked" {% endif %}{% if flags['favorite.disabled'] %}disabled {% endif %}  /><label for="favorite"></label>{{ lang.addnews['add_favorite'] }}<br />

 <input name="flag_HTML" type="checkbox" class="check" id="flag_HTML" value="1" {% if (flags['html.disabled']) %}disabled {% endif %} {% if (flags['html']) %}checked="checked"{% endif %}/> <label for="flag_HTML"></label>{{ lang.addnews['flag_html'] }}<br />
 <input type="checkbox" name="flag_RAW" value="1" class="check" id="flag_RAW" {% if (flags['html.disabled']) %}disabled {% endif %} {% if (flags['raw']) %}checked="checked"{% endif %}/><label for="flag_RAW"></label>{{ lang.addnews['flag_raw'] }}<br />
